...
// Confirmed from live capture (file.txt):
//   CDN = storage.googleapiscdn.com
//   M3U8 = https://storage.googleapiscdn.com/chunks/{24-hex}/original/index.m3u8
//   Segments end with .html (fine for ExoPlayer - content-type is video/mp2t)
//   Referer = empty (confirmed from captured headers)
...
object Crypto {
    ...
    fun decryptToUrl(enc: String?): String? {
        ...
        for (pass in PASSES) {
            ...
            if (isStreamUrl(r)) { println("AVS-CRYPTO ok pass='${pass.take(8)}..'"); return r }
        }
        return null
    }
...
class AnimeVietSub : MainAPI() {
    ...
    // Confirmed from capture: CDN needs no Referer
    ...
    override suspend fun loadLinks(
        ...
        // Strategy A: AJAX (main flow) — 20s
        var found = withTimeoutOrNull(20_000L) {
            ajaxFlow(epUrl, filmId, ids.toList(), cookies, callback)
        } ?: false
        log("A", "found=$found")

        // Strategy B: CDN probe — scan page for MongoDB ObjectIDs → verify via HTTP
        // Only runs if AJAX failed. Verifies URL exists before emitting.
        if (!found) found = withTimeoutOrNull(12_000L) {
            cdnProbeStrategy(body, epUrl, callback)
        } ?: false
        log("B", "found=$found")

        // Strategy C: page scrape (iframes, video tags) — only known hosts
        ...
        for (epId in ids.take(6)) {
            try {
                ...
                val j1 = try {
                    ...
                    if (handleBody(body1, epUrl, cb)) return true; continue
                }
                ...
                if (lnk is List<*>) {
                    for (item in lnk.filterIsInstance<Map<String, Any?>>()) {
                        ...
                        if (emitStream(u, epUrl, cb)) return true
                    }
                }

                // HTML with server buttons → Step 2
                ...
                if (buttons.isEmpty()) { if (handleBody(htmlStr, epUrl, cb)) return true; continue }
                ...
                for (btn in buttons) {
                    ...
                    val paramSets = buildList {
                        ...
                        add(mapOf("link" to hash, "play" to play.ifBlank { "api" }, "id" to btnId, "backup" to "1"))
                    ...
                    for (params in paramSets) {
                        try {
                            ...
                            if (handleBody(body2, epUrl, cb)) return true
    ...
    // ── handleBody: extract stream URL from any response text ─────────────────
    // KEY FIX: does NOT emit unless URL is definitely a stream
    private suspend fun handleBody(body: String, ref: String, cb: (ExtractorLink) -> Unit): Boolean {
        if (body.isBlank()) return false

        // 1. Direct CDN URL in response (most reliable)
        val cdnUrl = extractCdnUrl(body)
        if (cdnUrl != null) { log("HDL_CDN", cdnUrl); if (emitStream(cdnUrl, ref, cb)) return true }

        // 2. Any .m3u8 / .mp4 URL embedded in response
            ...
            for (key in listOf("link", "url", "stream", "src", "file", "m3u8")) {
                ...
                when {
                    ...
                    v is String && v.isNotBlank() -> {
                        val dec = Crypto.decryptToUrl(v)
                        log("HDL_DEC", "$key → $dec")
                        if (dec != null && emitStream(dec, ref, cb)) return true
                    }
        ...
        // 4. Whole body is encrypted
        if (body.length > 20 && body.trimStart().first().let { it != '{' && it != '[' && it != '<' }) {
            val dec = Crypto.decryptToUrl(body.trim())
            log("HDL_BODY_DEC", dec ?: "null")
            if (dec != null && emitStream(dec, ref, cb)) return true
        }
        return false
    }

    // ── Strategy B: CDN probe — verify via HTTP before emitting ───────────────
    // This is the CRITICAL FIX: we verify the URL exists before emitting.
    // Prevents ExoPlayer from spinning on non-existent URLs.
            ...
            for (suffix in listOf("index.m3u8", "playlist.m3u8", "master.m3u8")) {
                ...
                try {
                    ...
                    if (r.code == 200 && text.contains("#EXTM3U")) {
                        log("CDN_HIT", url)
                        // URL verified: safe to emit
                        if (emitVerifiedM3u8(url, epUrl, cb)) return true
                    }
                } catch (_: Exception) {}
            }
        }
    ...
    private suspend fun encMineStrategy(body: String, epUrl: String, cb: (ExtractorLink) -> Unit): Boolean {
        for (m in Regex("""(?:file|link|source|url|enc|m3u8)\s*[:=]\s*["']([A-Za-z0-9+/=]{40,})["']""").findAll(body)) {
            ...
            log("ENC_MINE", dec)
            if (emitStream(dec, epUrl, cb)) return true
        }
        return false
    }
    ...
    // Extract CDN URL from a response body (returns index.m3u8 URL)
    private fun extractCdnUrl(body: String): String? {
        // Full CDN URL present
    ...
    // Extract direct .m3u8 / .mp4 URLs from a body
    ...
    // emitStream: only emits if URL is definitively a stream URL
    // Returns false for anything uncertain — prevents "load forever" from bad URLs
    private suspend fun emitStream(url: String, ref: String, cb: (ExtractorLink) -> Unit): Boolean {
        if (!url.startsWith("http")) return false
        log("EMIT", url.take(120))
        return when {
            url.contains(CDN) -> {
                // CDN URL: construct index.m3u8 and emit (already verified in cdnProbeStrategy
                // or came directly from AJAX response — trust AJAX responses)
                ...
                val m3u8 = if (hexId != null) "https://$CDN/chunks/$hexId/original/index.m3u8"
                           ...
                           else return false // Don't emit CDN URLs we can't resolve to m3u8
            ...
            else -> false // Don't emit unknown URL types — prevents spin forever
        }
    }

    // emitVerifiedM3u8: verify URL is real before emitting
    // CRITICAL FIX: avoids ExoPlayer spinning on non-existent M3U8
