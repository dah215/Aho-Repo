...
// ═══════════════════════════════════════════════════════════════════════
// Architecture confirmed from live network capture (file.txt):
//   CDN = storage.googleapiscdn.com
//   M3U8 = https://storage.googleapiscdn.com/chunks/{24-hex-id}/original/index.m3u8
//   Segments end with .html (obfuscation, works fine with ExoPlayer)
//   No special Referer needed for CDN
// ═══════════════════════════════════════════════════════════════════════
...
object Crypto {
    ...
    fun isStreamUrl(s: String?): Boolean {
        ...
        return s.contains("googleapiscdn.com") ||
               ...
               s.contains("/stream/") || s.contains("playlist")
    }
}
...
class AnimeVietSub : MainAPI() {
    ...
    // CDN confirmed: no Referer needed (captured from live session with empty Referer)
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        if (epNodes.isEmpty()) {
            val wUrl = doc.selectFirst(
                "a.btn-see,a[href*='/tap-'],a[href*='/episode-'],.btn-watch a"
    ...
    override suspend fun loadLinks(
        ...
        val pageRes = try { app.get(epUrl, interceptor = cf, headers = pageH) }
                      catch (e: Exception) { log("ERR", e.message ?: ""); return false }
        val body    = pageRes.text ?: return false
        val cookies = pageRes.cookies.toMutableMap()

        // ── Immediate CDN URL in page body ────────────────────────────────
        extractCdnM3u8(body)?.let {
            log("CDN_PAGE", it)
            if (emitM3u8(it, epUrl, callback)) return true
        }

        // ── Collect episodeId candidates ──────────────────────────────────
        ...
        Regex("""(?:episodeId|episode_id|epId|filmEpisodeId)\s*[=:]\s*["']?(\d+)["']?""")
            .findAll(body).map { it.groupValues[1] }.forEach { ids.add(it) }
        ...
        // ── Strategy C: mine hex IDs → try CDN ───────────────────────────
        if (!found) found = withTimeoutOrNull(10_000L) {
            hexMineStrategy(body, epUrl, callback)
        } ?: false
        log("C", "found=$found")

        // ── Strategy D: mine encrypted blobs ─────────────────────────────
        if (!found) found = withTimeoutOrNull(8_000L) {
    ...
    private suspend fun ajaxFlow(
        ...
        for (epId in ids.take(6)) {
            ...
            try {
                ...
                log("AJAX1", "code=${r1.code} body=${body1.take(400)}")
                ...
                val j1 = try {
                    ...
                    // Not JSON: try direct processing
                    if (processBody(body1, epUrl, cb)) return true
                    continue
                }
                log("AJAX1", "keys=${j1.keys}")

                // Direct link field
                val lnk = j1["link"] ?: j1["url"] ?: j1["stream"]
                if (lnk != null) {
                    val url = when {
                        lnk is String && lnk.startsWith("http") -> lnk
                        lnk is String -> Crypto.decryptToUrl(lnk)
                        else -> null
                    }
                    if (url != null) { log("DEC", url); if (handleStreamUrl(url, epUrl, cb)) return true }

                    // Array of {file} objects
                    if (lnk is List<*>) {
                        for (item in lnk.filterIsInstance<Map<String, Any?>>()) {
                            val f = (item["file"] ?: item["src"] ?: item["url"])?.toString() ?: continue
                            val u = if (f.startsWith("http")) f else Crypto.decryptToUrl(f) ?: continue
                            if (handleStreamUrl(u, epUrl, cb)) return true
                        }
                    }
                }

                // HTML → buttons → step 2
                val rawHtml = j1["html"]?.toString()
                    ?: if (!body1.trimStart().startsWith("{")) body1 else ""
                if (rawHtml.isNotBlank()) {
                    // Immediate CDN in html
                    extractCdnM3u8(rawHtml)?.let {
                        log("CDN_HTML", it)
                        if (emitM3u8(it, epUrl, cb)) return true
                    }
                    val buttons = Jsoup.parse(rawHtml).select(
                        "a.btn3dsv,a[data-href],a[data-play],a[data-link]," +
                        ".server-item a,.btn-server,li[data-id] a,.episodes-btn a,button[data-href]"
                    )
                    log("AJAX1", "buttons=${buttons.size}")
                    for (btn in buttons) {
                        val hash = listOf("data-href","data-link","data-url")
                            .firstNotNullOfOrNull { btn.attr(it).trim().takeIf { v -> v.isNotBlank() && !v.startsWith("http") && v != "#" } }
                            ?: run {
                                // maybe it's a direct URL
                                val h = btn.attr("data-href").trim().ifBlank { btn.attr("href").trim() }
                                if (h.startsWith("http") && handleStreamUrl(h, epUrl, cb)) return true
                                continue
                            }
                        val play  = btn.attr("data-play").trim()
                        val btnId = btn.attr("data-id").filter { it.isDigit() }.ifBlank { epId }
                        log("AJAX2", "hash=${hash.take(60)} play=$play btnId=$btnId")

                        val paramSets = listOf(
                            mapOf("link" to hash, "id" to btnId, "backup" to "1"),
                            mapOf("link" to hash, "play" to play.ifBlank { "api" }, "id" to btnId, "backup" to "1"),
                            mapOf("link" to hash, "id" to epId, "backup" to "1"),
                            if (filmId.isNotBlank()) mapOf("link" to hash, "id" to filmId) else null
                        ).filterNotNull()
                        ...
                        for (params in paramSets) {
                            try {
                                ...
                                log("AJAX2", "code=${r2.code} body=${body2.take(400)}")
                                if (processBody(body2, epUrl, cb)) return true
                            } catch (e: Exception) { log("AJAX2", "err: ${e.message}") }
                        }
                    }
                }
            } catch (e: Exception) { log("AJAX", "err id=$epId: ${e.message}") }
        }
        return false
    }

    // ── processBody: handle any response body ────────────────────────────────
    private suspend fun processBody(body: String, ref: String, cb: (ExtractorLink) -> Unit): Boolean {
        if (body.isBlank()) return false

        // CDN pattern — highest priority
        extractCdnM3u8(body)?.let { if (emitM3u8(it, ref, cb)) return true }

        // Direct URLs in body
        for (u in extractUrls(body)) { if (handleStreamUrl(u, ref, cb)) return true }
        ...
        try {
            ...
            for (key in listOf("link","url","stream","src","file","m3u8")) {
                ...
                when {
                    v is String && v.startsWith("http") -> if (handleStreamUrl(v, ref, cb)) return true
                    v is String -> { val dec = Crypto.decryptToUrl(v); if (dec != null && handleStreamUrl(dec, ref, cb)) return true }
                    v is List<*> -> for (item in v.filterIsInstance<Map<String, Any?>>()) {
                        val f = (item["file"] ?: item["src"] ?: item["url"])?.toString() ?: continue
                        val u = if (f.startsWith("http")) f else Crypto.decryptToUrl(f) ?: continue
                        if (handleStreamUrl(u, ref, cb)) return true
                    }
                }
            }
        } catch (_: Exception) {}

        // Whole body is encrypted
        if (body.length > 20 && !body.trimStart().first().let { it == '{' || it == '[' || it == '<' }) {
            val dec = Crypto.decryptToUrl(body.trim())
            if (dec != null) { log("DEC_BODY", dec); if (handleStreamUrl(dec, ref, cb)) return true }
        }
        return false
    }
    ...
    private suspend fun scrapeStrategy(
        ...
        for (el in doc.select("video source[src],video[src]")) {
            fix(el.attr("src"))?.let { if (handleStreamUrl(it, epUrl, cb)) return true }
        }
        for (el in doc.select("[data-file],[data-url],[data-source],[data-stream]")) {
            ...
            val u = if (raw.startsWith("http")) raw else Crypto.decryptToUrl(raw) ?: continue
            if (handleStreamUrl(u, epUrl, cb)) return true
        }
        return false
    }

    // ── Strategy C: MongoDB hexId probe against CDN ───────────────────────────
    private suspend fun hexMineStrategy(body: String, epUrl: String, cb: (ExtractorLink) -> Unit): Boolean {
        val hexIds = Regex("""[0-9a-f]{24}""").findAll(body).map { it.value }.toSet()
        log("HEX", "hexIds=$hexIds")
        for (hexId in hexIds.take(8)) {
            for (suffix in listOf("index.m3u8","playlist.m3u8","master.m3u8")) {
                ...
                try {
                    ...
                    if (r.code == 200 && r.text?.contains("#EXTM3U") == true) {
                        log("HEX_HIT", url)
                        if (emitM3u8(url, epUrl, cb)) return true
                    }
                } catch (_: Exception) {}
            }
        }
    ...
    // ── Strategy D: encrypted blob mining ────────────────────────────────────
    private suspend fun encMineStrategy(body: String, epUrl: String, cb: (ExtractorLink) -> Unit): Boolean {
        for (m in Regex("""(?:file|link|source|url|enc|m3u8)\s*[:=]\s*["']([A-Za-z0-9+/=]{40,})["']""").findAll(body)) {
            val dec = Crypto.decryptToUrl(m.groupValues[1]) ?: continue
            log("ENC_MINE", dec)
            if (handleStreamUrl(dec, epUrl, cb)) return true
        }
        return false
    }

    // ── CDN URL extraction ────────────────────────────────────────────────────
    // Converts any storage.googleapiscdn.com reference to an index.m3u8 URL
    private fun extractCdnM3u8(body: String): String? {
        // Full CDN URL
        Regex("""https?://storage\.googleapiscdn\.com/chunks/([0-9a-f]{24})/[^\s"'<>\\]+""")
            .find(body)?.let { m ->
                val hexId = Regex("""/chunks/([0-9a-f]{24})/""").find(m.value)?.groupValues?.get(1) ?: return null
                return if (m.value.contains(".m3u8")) m.value
                else "https://$CDN/chunks/$hexId/original/index.m3u8"
            }
        // Just hexId near googleapiscdn mention
        if (body.contains("googleapiscdn")) {
            Regex("""[0-9a-f]{24}""").find(body)?.groupValues?.get(0)?.let { hexId ->
                return "https://$CDN/chunks/$hexId/original/index.m3u8"
            }
        }
        return null
    }
    ...
    // ── handleStreamUrl: route CDN vs regular URLs ────────────────────────────
    private suspend fun handleStreamUrl(url: String, ref: String, cb: (ExtractorLink) -> Unit): Boolean {
        if (!url.startsWith("http")) return false
        log("HANDLE", url.take(120))
        if (url.contains(CDN)) {
            // If partial storage URL without m3u8, probe index.m3u8
            return if (url.contains(".m3u8") || url.contains("index") || url.contains("playlist")) {
                emitM3u8(url, ref, cb)
            } else {
                val hexId = Regex("""/chunks/([0-9a-f]{24})/""").find(url)?.groupValues?.get(1)
                if (hexId != null) {
                    val m3u8 = "https://$CDN/chunks/$hexId/original/index.m3u8"
                    log("CDN_CONSTRUCT", m3u8)
                    emitM3u8(m3u8, ref, cb)
                } else emitM3u8(url, ref, cb)
            }
        }
        return when {
            url.contains(".m3u8") || url.contains("/hls/") -> emitM3u8(url, ref, cb)
            url.contains(".mp4") || url.contains(".mkv") || url.contains(".webm") -> {
                try { cb(newExtractorLink(name, name, url) {
            ...
            else -> try { loadExtractor(url, ref, {}, cb); true } catch (_: Exception) { false }
        }
    }
