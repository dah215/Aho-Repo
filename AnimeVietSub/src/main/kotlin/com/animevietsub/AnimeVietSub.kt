...
class AnimeVietSubProvider : MainAPI() {
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        // THE FINAL SOLUTION: Sử dụng WebView để xử lý Redirect và bắt link m3u8 từ Host mới stream.googleapiscdn.com
        val playlistUrl = capturePlaylistUrlFinalSolution(epUrl) ?: return true

        // SỬA LỖI 2004: Sử dụng bộ tiêu đề Authorized Origin cực kỳ khắt khe
        val playlistHost = java.net.URI(playlistUrl).host
        
        callback(newExtractorLink(source = name, name = "$name - Final Solution", url = playlistUrl, type = ExtractorLinkType.M3U8) {
            this.quality = Qualities.P1080.value
            this.headers = mapOf(
                "User-Agent" to UA,
                "Referer" to "$mainUrl/", // BẮT BUỘC: Referer phải là trang web chính thức
                "Origin" to mainUrl,      // BẮT BUỘC: Origin phải là trang web chính thức
                "Accept" to "*/*",
                "Accept-Language" to "vi-VN,vi;q=0.9",
                "Sec-Fetch-Dest" to "empty",
                "Sec-Fetch-Mode" to "cors",
                "Sec-Fetch-Site" to "cross-site", // BẮT BUỘC: cross-site vì CDN khác domain trang chính
    ...
    private suspend fun capturePlaylistUrlFinalSolution(epUrl: String): String? {
                ...
                suspendCancellableCoroutine { cont ->
                    ...
                    wv.webViewClient = object : WebViewClient() {
                        override fun shouldInterceptRequest(view: WebView, request: WebResourceRequest): WebResourceResponse? {
                            ...
                            // Đánh chặn link m3u8 từ Host mới stream.googleapiscdn.com
                        ...
                        override fun onPageFinished(view: WebView, url: String) {
                            // Giả lập tương tác để kích hoạt trình phát ArtPlayer
                            val jsTrigger = """
                                (function() {
                                    var playBtn = document.querySelector('.art-video-control-play') || document.querySelector('.art-state') || document.body;
                                    if (playBtn) {
