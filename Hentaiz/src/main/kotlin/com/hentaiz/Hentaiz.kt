...
class HentaiZProvider : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        // Sửa lỗi hiển thị mô tả bị mã hóa Unicode
    ...
    override suspend fun loadLinks(
        ...
        if (!sonarUrl.isNullOrBlank() && videoId != null) {
            ...
            // 2. Gọi API trực tiếp của Sonar (Giả lập hành động của Player)
            // Thay vì quét HTML, ta gọi API để lấy JSON cấu hình chuẩn
            ...
            try {
                val apiHeaders = mapOf(
                    ...
                    "X-Requested-With" to "XMLHttpRequest", // Quan trọng: Giả lập AJAX request
                    "Content-Type" to "application/x-www-form-urlencoded"
                )

                // Gửi POST request để lấy link (thường các player dùng POST)
                ...
                if (apiRes.code == 200) {
                        ...
                        dataArray.forEach { item ->
                            ...
                            callback(
                                newExtractorLink(
                                    ...
                                    this.referer = fixedSonarUrl // Referer phải là trang Player
                    ...
                    // Fallback: Nếu API POST thất bại, thử quét Regex trong trang Player như cũ
                    val sonarPageRes = app.get(fixedSonarUrl, headers = mapOf("Referer" to "$mainUrl/"))
                    val sonarHtml = sonarPageRes.text
                    
                    // Regex tìm file: "..." (Cấu hình JW Player)
                    val fileRegex = """file\s*:\s*["']([^"']+)["']""".toRegex()
                    fileRegex.findAll(sonarHtml).forEach { match ->
